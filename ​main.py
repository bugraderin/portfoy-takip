...
with tab_portfoy:
    ...
    if data_p:
        ...
        # SIRALI METRİKLER (Küçük Boyutlu)
        varlik_listesi = []
...
with tab_gelir:
    ...
    if data_gelir:
        ...
        df_g_list.columns = df_g_list.columns.str.strip() # Gizli boşlukları temizler
        
        beklenen = ["Maaş", "Prim", "Yatırım"]
        mevcut = [col for col in beklenen if col in df_g_list.columns]
        
        for col in mevcut:
            df_g_list[col] = pd.to_numeric(df_g_list[col], errors='coerce').fillna(0)
        
        if len(df_g_list) > 0 and mevcut:
            son_g = df_g_list.iloc[-1]
            g_pie_data = pd.DataFrame({'Kat': mevcut, 'Tut': [son_g[c] for c in mevcut]})
            g_pie_data = g_pie_data[g_pie_data['Tut'] > 0]
            
            if not g_pie_data.empty:
                st.plotly_chart(px.pie(g_pie_data, values='Tut', names='Kat', hole=0.4, title="Son Ay Gelir Dağılımı"), use_container_width=True)
            else:
                st.info("Henüz görselleştirilecek bir gelir tutarı yok.")
