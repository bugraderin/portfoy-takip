...
with st.sidebar:
    ...
    with st.form("veri_formu", clear_on_submit=True):
        ...
        for e in enstrumanlar:
            # value=None ve label_visibility ile butonları/varsayılan 0'ı kaldırdık
            inputs[e] = st.number_input(
                f"{enstruman_bilgi[e]} {e} (TL)", 
                min_value=0.0, 
                step=1.0, 
                value=None,  # 0 ön tanımlı gelmez, boş gelir
...
# CSS ile yan taraftaki + ve - butonlarını gizleme (Tarayıcı bazlı okları kaldırır)
st.markdown("""
    <style>
    /* Chrome, Safari, Edge, Opera - Okları kaldır */
    input[::-webkit-outer-spin-button],
    input[::-webkit-inner-spin-button] {
      -webkit-appearance: none;
      margin: 0;
    }
    /* Firefox - Okları kaldır */
    input[type=number] {
...
if submit:
    # Boş bırakılan yerleri 0 olarak kaydet
...
if data:
    ...
    # --- 6. PERFORMANS ANALİZİ (GELİŞTİRİLDİ) ---
    st.divider()
    ...
    # Toplam Büyüme/Küçülme Hesaplama
    ...
    # Bilgi Kutusu
    renk = "green" if toplam_degisim >= 0 else "red"
    st.markdown(f"""
        <div style="padding:15px; border-radius:10px; border: 1px solid #ddd; background-color: #f9f9f9;">
            <b>{secim}</b> öncesine göre portföy durumu: <br>
            Değer: <span style="color:{renk};"><b>{baslangic['Toplam']:,.0f} TL</b></span> → <b>{guncel['Toplam']:,.0f} TL</b> <br>
            Değişim: <span style="color:{renk};"><b>%{toplam_yuzde:.2f}</b></span> ({toplam_degisim:,.0f} TL)
        </div>
    """, unsafe_allow_html=True)
    
    st.write("")
    
    # Varlık Bazlı Performans Kartları
